...
const PropertyEdit = (props) => {
    ...
    const propertyUpdate = (event, property) => {
        ...
        fetch(`http://localhost:4000/property/${props.propertyToUpdate.id}`,{
            method: 'PUT',
            body: JSON.stringify({category: editCat, name: editName, year: editYear, model: editModel, serial: editSerial, imgURL: editImgURL, value: editValue}),
            headers: new Headers({
    ...
    return(
        <Modal isOpen={true}>
            <ModalHeader>Log a Property</ModalHeader>
            <ModalBody>
                <Form onSubmit={propertyUpdate}>
                    <FormGroup>
                        <Label htmlFor="category">Edit Category:</Label>
                        <Input type="select" name="category" value={editCat} onChange={(e) => setEditCat(e.target.value)}>
                    ...
                    <FormGroup>
                        ...
                        <Input type="text" name="year" value={editYear} onChange={(e) => setEditYear(e.target.value)}>
                        </Input>
                    </FormGroup>
                    <FormGroup>
                        <Label htmlFor="model">Edit Model:</Label>
                        <Input type="text" name="model" value={editModel} onChange={(e) => setEditModel(e.target.value)}>
                        </Input>
                    </FormGroup>
                    <FormGroup>
                        <Label htmlFor="serial">Edit Serial:</Label>
                        <Input type="text" name="serial" value={editSerial} onChange={(e) => setEditSerial(e.target.value)}>
                        </Input>
                    </FormGroup> 
                    <FormGroup>
                        <Label htmlFor="imgURL">Edit Image:</Label>
                        <Input type="text" name="imgURL" value={editImgURL} onChange={(e) => setEditImgURL(e.target.value)}>
                        </Input>
                    </FormGroup>
                    <FormGroup>
                        <Label htmlFor="value">Edit Value:</Label>
                        <Input type="text" name="value" value={editValue} onChange={(e) => setEditValue(e.target.value)}>
                        </Input>
                    </FormGroup>
                    <Button type="submit">Update the property!</Button>
                </Form>
            </ModalBody>
        </Modal>
    )
